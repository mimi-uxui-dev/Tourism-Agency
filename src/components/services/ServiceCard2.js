...
function ServiceCard2({ data, ready }) {
     ...
     const getImage = (type) => {
          if (data.hasOwnProperty('offers_media')) {
               console.log(data.offers_media.filter(e => e.type__en === type))
               const x = data.offers_media.filter(e => e.type__en === type)
          ...
          <div className='serviceCard'>
               <div>
                    <div className="img_container">
                         <Link to={{ pathname: `/offers/${data.id}`, state: { data: data } }}>
                              <ReactPlaceholder showLoadingAnimation={true} type='text' ready={ready} rows={1} color='#CFCFCF'>
                                   {
                                        data.hasOwnProperty('offers_media') && data.offers_media.length === 0 ? <>
               ...
               <ReactPlaceholder showLoadingAnimation={true} type='text' ready={ready} rows={5} color='#CFCFCF'>
                    <div className='text' >
                         ...
                         <div className="cardContent">
                              <h3>
                                   {
                                        data.hasOwnProperty('title__en') ?
                                             <Link to={{ pathname: `/offers/${data.id}`, state: { data: data } }}> {cookies.get('i18next') === 'ar' ? data.title__ar : (cookies.get('i18next') === 'fr' ? data.title__fr : data.title__en)}  </Link>
                                             :
                         ...
                         <div className='card_days_price'>
                              ...
                              <div className='price'> {t('from')}  {data.price} DZD  </div>
                         </div>
                    </div>
               </ReactPlaceholder>
          </div>
